...
async function createAdminUser() {
  try {
    ...
    const { data: existingAdmin, error: checkError } = await supabase
      .from('admin_users')
      .select('*')
      .eq('email', 'admin@researchhub.com')
      .single();
    ...
    if (authData.user) {
      ...
      console.log('📧 Email: admin@researchhub.com');
...
if (typeof window !== 'undefined') {
  ...
  console.log('🌱 Database seeding function available! Run: seedDatabase()');
  console.log('👑 Admin user creation function available! Run: createAdminUser()');
}
